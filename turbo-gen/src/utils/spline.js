import { okLabToSRGB } from './perceptual';

export const KNOT_POSITIONS = [0, 1 / 6, 2 / 6, 3 / 6, 4 / 6, 5 / 6, 1];
export const NUM_KNOTS = 7;
...
export function evaluateSpline(u, knotValues, channel) {
...
export function evaluateRGB(u, knotValues) {
...
export function getClipAmount(u, knotValues) {
    const r = evaluateSpline(u, knotValues, 0);
    const g = evaluateSpline(u, knotValues, 1);
    const b = evaluateSpline(u, knotValues, 2);
...
export function initializeKnotsFromRainbow() {
...
export function initializeKnots(seed) {
...
export function initializeKnotsRandom(seed) {
    const rng = seededRandom(seed);
    const knots = [];
